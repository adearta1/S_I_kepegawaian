    ...
    <div class="container pt-5 pb-5" style="margin-top : 4rem;">
        <div class="row justify-content-md-center ">
            ...
            <div class="card shadow sm-3 mb-5 bg-white rounded">
                <div class="card-header bg-primary mb-0"><h5 class="text-center text-white">Edit Data <span class="font-weight-bold text-light " >Pendidikan</span></h5></div>
                <div class="card-body">
                <?php
                    include_once("../../koneksi.php");
                    if (isset($_GET['id_pendidikan'])) {
                        $ubah=$_GET['id_pendidikan'];
                    } else {
                        die ("ERROR");
                    }
                    
                    $da = mysqli_query($koneksi,"SELECT * FROM pendidikan WHERE id_pendidikan='$ubah'");
                    if (isset($_POST['edit'])) {
                        ...
                        if($data->num_rows > 0){
                          $query="UPDATE pendidikan SET id_pendidikan = '$id', NIK='$NIK',pendidikan='$pendidikan',jurusan='$jurusan', update_at='$update_at' WHERE id_pendidikan='$id'";
                          mysqli_query($koneksi,$query) or die ('gagal');
                          echo '<script>alert("Berhasil mengedit data."); document.location="../../pendidikan.php";</script>';
                        } else {
                        ?>
                        <div class="alert alert-primary text-center text-danger" role="alert">
                            gagal mengapdate data pendidikan
                        </div> 
                        <?php } 
                ...
                <form action="" method="post">
                    <div class="form-group">
                        ...
                        <input type="hidden" name="id_pendidikan" class="form-control" value="<?php echo $dat['id_pendidikan']; ?>">
                        <input type="text" name="NIK" class="form-control" value="<?php echo $dat['NIK']; ?>">
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlSelect1">Pendidikan</label>
                        <input type="text" name="pendidikan" class="form-control" value="<?php echo $dat['pendidikan']; ?>">
                    </div>
                    <div class="form-group">
                        <label for="exampleFormControlSelect1">Jurusan</label>
                        <input type="text" name="jurusan" class="form-control" value="<?php echo $dat['jurusan']; ?>">
                    </div>
                    <div class="form-group">
                        <input type="submit" name="edit" value="Simpan" class="btn btn-primary btn-block">
                        <a href="../../pendidikan.php" class="btn btn-danger btn-block active" role="button" aria-pressed="true">Batal</a>
                    </div>
                </form>
                <?php }?>
                </div>
